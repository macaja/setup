...
- Plan structure: Diagrama (see below), Context (what this deliverable is, its design sources, agreed decisions, explicit out-of-scope list), Cambios (numbered, per package/module, concrete: files, symbols, patterns to follow), Tests, Riesgos/bordes, Verificación. Match the language of the feature context (Spanish context → Spanish plan; code identifiers stay as-is).
- Diagrama: the plan file opens with a Mermaid block (fenced as \`\`\`mermaid) showing where the change is going — code flow, module relationships, data flow, or a sequence of calls. Use `flowchart TD` for control flow, module relationships, and decision trees; `sequenceDiagram` when the order of calls between parties is the point; `erDiagram` for tables and their relationships. Keep each diagram under 15 nodes — split at a natural seam (happy path vs error handling) rather than shrinking labels. Label an edge only when the label carries information (`writes`, `on 401`). If the change only makes sense against what exists today, show both, two stacked diagrams, labelled. Skip the diagram when there is nothing structural to show; a single box is noise.
